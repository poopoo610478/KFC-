...
export const useCouponStore = defineStore("couponStore", {
  ...
  actions: {
    ...
    async fetchCoupons() {
      try {
        const response = await fetch("http://localhost:8080/api/coupons/all");
        if (response.ok) {
    ...
    async updateCoupon(coupon) {
      try {
        ...
        const response = await fetch(
          `http://localhost:8080/api/coupons/update/${coupon.value.couponId}`,
          {
            method: "PUT",
            body: formData,
          }
    ...
    async deleteCoupon(id) {
      try {
        const response = await fetch(
          `http://localhost:8080/api/coupons/delete/${id}`,
          {
            method: "DELETE",
          }
        );
...
//查詢優惠券
    ...
    async fetchAllImages() {
      try {
        const response = await fetch("http://localhost:8080/api/coupons/images/all", {
